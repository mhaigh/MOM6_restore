...
!                 call to user_surface_forcing_init
  ...
  integer :: j_shift, j_50, Jq_50
  real :: y_shift, yf, yfv
  real :: m, tau, PI, len_lat, asym
  real :: x, y
  ...
  j_50 = int((je - js) / 2) + js
  Jq_50 = int((Jeq - Jsq) / 2) + Jsq

  yf = G%geoLatCu(Isq,j_50)
  yfv = G%geoLatCv(is,Jq_50)
  len_lat = G%geoLatCu(is,je) - G%GeoLatCu(is,js) ! 2. * yf
  ...
  tau = 0.09 ! was 0.08
  m = 0.1
  asym = 1.0 ! Asymmetry of the wind

  j_shift = 0 !Due to asymmetry 18 is suitable for 513 grid points. 
  y_shift = G%geoLatCu(is,j_shift+1) - G%geoLatCu(is,1)
  ...
  do j=js,je ; do I=is-1,Ieq	
    ...
    x = G%geoLonCu(I,j)
    y = G%geolatCu(I,j)
    !if (m * x - y + yf + y_shift < 0) then
    !  forces%taux(I,j) = G%mask2dCu(I,j) * 1.0 * tau * 0.5 * (1.0 + cos(2.0 * PI * (m * x - y + yf + y_shift) / (1.1*len_lat)))
    !else
    !  forces%taux(I,j) = G%mask2dCu(I,j) * 1.0 * tau * 0.5 * (1.0 + cos(2.0 * PI * (m * x - y + yf + y_shift) / (1.1*asym*len_lat)))
    !endif
    forces%taux(I,j) = G%mask2dCu(I,j) * 1.0 * tau * 0.5 * (1.0 + cos(2.0 * PI * (m * x - y + yf) / ((1.+m)*len_lat)) + asym * (y - G%south_lat) / len_lat)
  enddo ; enddo
  do J=js-1,Jeq ; do i=is,ie
    !ii = i !+ is + (pe * (nx - 1))
    x = G%geoLonCv(i,J)
    y = G%geoLatCv(i,J)
    !if (m * x - y + yf + y_shift < 0) then
    !  forces%tauy(i,J) = G%mask2dCv(i,J) * tau * m * 0.5 * (1.0 + cos(2.0 * PI * (m * x - y + yfv + y_shift) / (1.1*len_lat)))
    !else
    !  forces%tauy(i,J) = G%mask2dCv(i,J) * tau * m * 0.5 * (1.0 + cos(2.0 * PI * (m * x - y + yfv + y_shift) / (1.1*asym*len_lat)))
    !endif
    forces%tauy(I,j) = G%mask2dCu(I,j) * 1.0 * tau * m * 0.5 * (1.0 + cos(2.0 * PI * (m * x - y + yf) / ((1.+m)*len_lat)) + asym * (y - G%south_lat) / len_lat)
  enddo ; enddo
