...
subroutine h_relax_user_tilt(h,LB,G,ni,nj)

  type(ocean_grid_type),                     intent(inout) :: G   !< The ocean's grid structure
  real, dimension(SZI_(G),SZJ_(G),SZK_(G)),  intent(inout)  :: h
  type(loop_bounds_type),              intent(in)     :: LB
  integer,                             intent(in)     :: nj,ni
  real, dimension(SZI_(G),SZJ_(G))			    :: h_relax
  real, dimension(SZI_(G),SZJ_(G))              :: h1_restore    !! USER-added variable, used to store the profile towards which the upper layer thickness is restored. 
  ...
  real :: m ! These ought to match with values in user_surface_forcing.F90 if using both relaxation and USER wind forcing.
  real :: xf, yf, y_shift, L
  ...
  ig_50 = int((G%ieg - G%isg) / 2) + G%isg
  ih_50 = int((ie - is) / 2) + is
  ...
  relax_const = 1.0e-5 !0.5e-5   ! This value is dimensionless, actual rate of relaxation is relax_const/dt. 1.0 for 256, 0.5 for 512, 2.0 for 128
  ...
  xf = G%geoLonT(ig_50,js);
  yf = G%geoLatT(is,j_50);
  L = (G%geoLatT(is,j_62) - G%geoLatT(is,j_37));

  h_relax(:,:) = 0.0
  do j=js,je ; do i=is,ie
    x = G%geoLonT(i,j)
    y = G%geoLatT(i,j)
    if (m * x + yf - 0.5 * L < y - y_shift .AND. y - y_shift < m * x + yf + 0.5 * L) then
      xy = 2.0 * pi * (m * x - y + y_shift + yf) / L 
      !sine = xy - xy**3 / 6.0 + xy**5 / 120.0 - xy**7 / 5040.0 + xy**9 / 362880.0 - xy**11 / 39916800.0 + xy**13 / 6227020800.0 - xy**15 / 1307674368000 + xy**17 / 3.55687428e14 - xy**19 / 1.216451e17
      sine = SIN(xy)
      h1_restore(i,j) = 300.0 + dh * sine
      ! Always apply the relaxation in the jet region.
      h_relax(i,j) = relax_const * (h1_restore(i,j) - h(i,j,1))
      h(i,j,1) = h(i,j,1) + h_relax(i,j)
    !elseif (x > G%geoLonT(ih_50,js)) then
    !if (x > ) then
    !else
    !  h_relax(i,j) = relax_const * (600.0 - h(i,j,1))
    !  h(i,j,1) = h(i,j,1) + h_relax(i,j)
    endif 
  enddo ; enddo

  !print *, G%geoLonT(is,js)
  !print *, G%geoLonT(ih_50,js)
  !print *, G%geoLonT(ie,js)
  !print *, 0

  !call create_hrelax_nc('h_relax.nc',ni,nj,10)
  !call write_hrelax_nc('h_relax.nc',ni,nj,h_relax)
  ! Output h_relax

  ! Testing.
  !h(i,j+j_shift,1) = h(i,j+j_shift,1) + relax_const * ((300.0 + a * h1_restore(i,j)) - h(i,j+j_shift,1)) 
  !h(i,j+j_shift,2) = h(i,j+j_shift,2) + relax_const * ((700.0 + b * h1_restore(i,j)) - h(i,j+j_shift,2)) 
  !h(i,j+j_shift,3) = h(i,j+j_shift,3) + relax_const * ((3000.0 + c * h1_restore(i,j)) - h(i,j+j_shift,3))
  !h(i,j+j_shift,2) = h(i,j+j_shift,2) + relax_const * ((700.0 + b * h1_restore(i,j)) - h(i,j+j_shift,2)) 
  !h(i,j+j_shift,3) = h(i,j+j_shift,3) + relax_const * ((3000.0 + c * h1_restore(i,j)) - h(i,j+j_shift,3))
  ! The following loop applies the relaxation to h across the whole range (i.e. relaxes h to zero outside of the jet).
  !do j=js,je ; do i=is,ie
  !  h(i,j,1) = h(i,j,1) + relax_const * (h1_restore(i,j) - h(i,j,1)) 
  !enddo ; enddo

end subroutine h_relax_user_tilt

subroutine h_mass_cons(h,LB,G)
  ...
  mass = 0.0
  do j = js,je ; do i = is,ie
    mass = mass + h(i,j,1) - 300.0      ! Total extra mass in the upper layer
  enddo ; enddo

  dm = mass / (ni * nj) ! Mass to be added to each grid point
  print *, dm
...
subroutine continuity_PPM(u, v, hin, h, uh, vh, dt, G, GV, CS, uhbt, vhbt, OBC, &
  ...
  !real, allocatable, dimension(:,:)    :: h_relax
  ...
  call h_relax_user_tilt(h,LB,G,ni,nj)
  call h_mass_cons(h,LB,G)
...
! Read nc file
subroutine read_hrelax_nc(file_name,ni,nj,nlayers,h_relax)
      
implicit none
     
include 'netcdf.inc'
    
integer ni,nj,nlayers,start(3),count(3),i
real*8 h_relax(ni,nj,nlayers)
      
character*(*) file_name
      
character*(*) h_name
parameter(h_name = 'h_relax')
...
retval = nf_inq_varid(ncid, h_NAME, h_varid)                !! Request varid
if (retval .ne. nf_noerr) call handle_err(retval)
...
retval = nf_get_var(ncid,h_varid,start,count,h_relax)
if (retval .ne. nf_noerr) call handle_err(retval)
...
end subroutine read_hrelax_nc

! Create nc file
!==================================================================
      
subroutine create_hrelax_nc(file_name,ni,nj,nt)
...
character*(*) h_name,x_name,y_name,t_name
parameter(h_name = 'h relax')
parameter(x_name = 'X')
parameter(y_name = 'Y')
parameter(t_name = 'Time')
      
integer x_dimid,y_dimid,dimids(3),ndims,t_dimid
parameter(ndims=3)
      
integer ncid,retval,h_varid
     
retval = nf_create(FILE_NAME, nf_share, ncid)                 !! Create netcdf file
if (retval .ne. nf_noerr) call handle_err(retval)
      
! Define dimensions
retval = nf_def_dim(ncid, Y_NAME, nj, y_dimid)
if (retval .ne. nf_noerr) call handle_err(retval)
...
retval = nf_def_var(ncid, h_NAME, NF_DOUBLE, NDIMS, dimids,h_varid)
if (retval .ne. nf_noerr) call handle_err(retval)
retval = nf_close(ncid)                                         !! Close file
if (retval .ne. nf_noerr) call handle_err(retval)

end subroutine create_hrelax_nc

! Write to nc file
!==================================================================
      
subroutine write_hrelax_nc(file_name,ni,nj,h_relax)
...
integer h_varid,start(3),count(3)

! print format of the file.
retval = nf90_open(FILE_NAME, nf_write, ncid)                     !! Open file in write mode
if (retval .ne. nf_noerr) call handle_err(retval)
...
retval = nf_put_vara_double(ncid, h_varid, start, count,h_relax)
if (retval .ne. nf_noerr) call handle_err(retval)
      
retval = nf_close(ncid)                                         !! Close NetCDF file                
if (retval .ne. nf_noerr) call handle_err(retval)
        
end subroutine write_hrelax_nc
