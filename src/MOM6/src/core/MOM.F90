...
subroutine step_MOM(forces, fluxes, sfc_state, Time_start, time_interval, CS)
  ...
  real, dimension(SZI_(CS%G),SZJ_(CS%G)) :: h_relax
  ...
  do n=1,n_max
    ...
    !if (CS%id_h  > 0) call post_data(id_hr, h_relax, CS%diag)
...
subroutine initialize_MOM(Time, param_file, dirs, CS, Time_in, offline_tracer_mode, input_restart_file)
  ...
  !real, pointer, dimension(:,:) :: h_relax
...
!  dG%g_Earth = GV%g_Earth
  ...
  call register_diags(Time, G, GV, CS, CS%ADp, CS%tv%C_p,id_hr)
...
subroutine register_diags(Time, G, GV, CS, ADp, C_p,id_hr)
  ...
  id_hr = register_diag_field('ocean_model', 'h_relax', diag%axesTL, Time, &
